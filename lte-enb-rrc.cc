...
/************************************************************************************************************************************************
	...
	Modified by Mohamed Shoeb & Ashraf Khresheh, April 2015
...
#include "ns3/random-variable-stream.h"			// Added for GPM
#include <math.h>								// Added for GPM
...
std::vector <GlobalPersistenceModule>Table;
std::vector <GlobalPersistenceModule>::iterator ii;
std::vector <GlobalPersistenceModule>::iterator ii_rowindex;
std::vector <double>Stored_Video_Playback_Rates;
std::vector <double>Stored_Video_Playback_Sizes;
std::vector <double>Stored_Video_Playback_Times;

void updateGPM(uint16_t m_imsi, uint16_t m_rnti, uint16_t m_cellid)
{
	// ----------------------------------------------------------------------------
	// When a UE is registered put a record in the Global Persistence Module
	// 1. Check if UE already exists in Global Persistence Module
	// 2. If UE record already exists, only update RNTI of new cell
	// 3. If UE record does not exist, create a record entry
	// ----------------------------------------------------------------------------

	       bool UErecordfound;
	       uint16_t temprnti, tempcellid;

	       // std::cout << "\n   [!] Current table size is " << Table.size();
	       ...
	       GlobalPersistenceModule tempdata;

	       if (UErecordfound == false)
	       {

	       double vsize, vtime, vrate;

		   tempdata.videonodeid = Table.size();                                          // Use this only to initially assign a playback video
		   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	     // length to a UE (in a random fashion). Note that this ID
		   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	   	 // *may* not corrospond to the actual node ID in the UE container
		   // Obtain video size from list
		   vsize = Stored_Video_Playback_Sizes.at(tempdata.videonodeid);
		   vtime = Stored_Video_Playback_Times.at(tempdata.videonodeid);
		   vrate = Stored_Video_Playback_Rates.at(tempdata.videonodeid);

	       tempdata.imsi = m_imsi;
	       tempdata.rnti = m_rnti;
	       tempdata.cellid_t = m_cellid;
	       tempdata.cellid_tminus1 = 0;					      						     // Cell ID 0 indicates no available information (no previous) at intialization
	       tempdata.V    = 1.25 * vrate;                                                 // streaming rate = 1.25 * playback rate
	       tempdata.D    = tempdata.V * 0.001;                							 // bits in 1 TTI
	       tempdata.R_t  = 0;                          		  							 // Initially received no. of bits at this UE is 0 bits
	       tempdata.R_tminus1 = 0;                   		  						     // Initially no bits received in previous TTI
	       tempdata.video_length = vsize; 		                                         // No. of bytes in video file
	       tempdata.video_remaining = tempdata.video_length;  							 // Initially no bits have been delivered
	       tempdata.video_duration = vtime;                                              // Video duration in minutes
	       tempdata.user_type = 0;                                                   	 // Initial this UE is normal
	       tempdata.wcqi_t    = 0;														 // Initialize current CQI with undetermined value (0)
	       tempdata.wcqi_tminus1 = 0;												 	 // Initialize previous CQI with undetermined value (0)
	       tempdata.schedule_start = 0;                                              	 // Green scheduler has decided how many subframes this user will receive this 1 second
	       tempdata.schedule_stop = 0;                                               	 // Flag to indicate user has received their portion of bits during this 1 second
	       tempdata.tti_count = 0;                                                       // This counter counts the number of subframes received thus far since schedule cycle started;
	       tempdata.tti_total = 0;                                                       // Total number of subframes to receive this 1 second schedule cycle.
	       tempdata.schedule_priority_index = 65535;
	       tempdata.handover_occured = false;

	       Table.insert(Table.end(), tempdata);
	       std::cout << "\n   [!] UE IMSI " << m_imsi << " and RNTI " << m_rnti << " >> record created ..";
	       std::cout << "\n   [!] UE IMSI " << m_imsi << " and RNTI " << m_rnti << " requested video of size " << vsize << ", duration " << vtime << " minutes, and playback rate " << vrate << " bytes/sec ..";
	       std::cout << "\n   [!] Streaming rate set to " << 1.25 * vrate << " bytes/s ..";

	       //std::cout << "\n   [!] RRC: New table size is " << Table.size() << "\n";

	       }
	       else
	       {

	       temprnti = (*ii_rowindex).rnti;					 // Store temporary info.
	       tempcellid = (*ii_rowindex).cellid_t;

	       (*ii_rowindex).rnti = m_rnti;                     // Update UE record info.
	       (*ii_rowindex).cellid_t = m_cellid;
	       (*ii_rowindex).cellid_tminus1 = tempcellid;

	       if ((*ii_rowindex).cellid_t != (*ii_rowindex).cellid_tminus1)
	       {
	    	   (*ii_rowindex).handover_occured = true;
	    	   (*ii_rowindex).user_type = 0;
	    	   std::cout << "\n   [!] UE IMSI " << m_imsi << " and RNTI " << m_rnti << " is now classified as normal ..";
	       }

	       if (temprnti != m_rnti)
	       {
	       std::cout << "\n   [!] CONNECTION_SETUP : UE IMSI " << m_imsi << " and old RNTI " << temprnti << " has been replace with new RNTI " << m_rnti << " >> record updated ..";
	       }
	       else
	       {
		   //std::cout << "\n   [!] CONNECTION_SETUP : UE IMSI " << m_imsi << " has the same old and new RNTI " << temprnti << " >> record updated ..";
	       }
...
UeManager::RecvHandoverRequestAck (EpcX2SapUser::HandoverRequestAckParams params)
{
  ...
  // Update the GPM
  updateGPM(m_imsi, m_rnti, m_rrc->m_cellId);
